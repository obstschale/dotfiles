...
# Get all local branches mergend into master, except master, and write them to a file
# so I can edit it to remove branche I want to keep and then savely delete them
alias prune-merged='git branch --merged master | grep -v "^[ *]*master$" > /tmp/merged-branches && vim /tmp/merged-branches && xargs git branch -d < /tmp/merged-branches'
